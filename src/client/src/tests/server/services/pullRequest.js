...
const testDataComments_withLICENSEComment = [{
    ...
    'body': '[![License Assistant check](http://cla_dev.ngrok.com/24456091/pull/71/badge)](http://cla_dev.ngrok.com/KharitonOff/testGithub) <br/>Please agree to our Contributor License Agreement in order to get your pull request merged.',
    'created_at': '2011-04-14T16:00:49Z',
    'updated_at': '2011-04-14T16:00:49Z',
    'html_url': 'https://github.com/octocat/Hello-World/pull/1#discussion-diff-1',
    'pull_request_url': 'https://api.github.com/repos/octocat/Hello-World/pulls/1',
...
const testDataComments_withoutLICENSE = [{
    'url': 'https://api.github.com/repos/octocat/Hello-World/pulls/comments/1',
    'id': 2,
    'diff_hunk': '@@ -16,33 +16,40 @@ public class Connection : IConnection...',
    'path': 'file1.txt',
...
describe('pullRequest:badgeComment', () => {
    ...
    it('should add 2 notes  to the comment if there is an external committer (corporate LICENSE) and he/she is not a GitHub User ', async () => {
        direct_call_data = testDataComments_withLICENSEComment
        assertionFunction = async (args) => {
            assert.equal(args.fun, 'updateComment')
            assert.equal(args.token, 'xyz')
    ...
    it('should add only one note (not a Github user) to the comment if there is no external committer (corporate LICENSE)', async () => {
        direct_call_data = testDataComments_withLICENSEComment
        assertionFunction = async (args) => {
            assert.equal(args.fun, 'updateComment')
            assert.equal(args.token, 'xyz')
    ...
    it('should add a note to the comment with name of ONE committer who has no github account', async () => {
        direct_call_data = testDataComments_withLICENSEComment
        assertionFunction = async (args) => {
            assert(args.arg.body.indexOf('**user1** seems not to be a GitHub user. You need a GitHub account to be able to sign the LICENSE. ') >= 0)
            return 'githubRes'
        }
    ...
    it('should add a note to the comment with names of MULTIPLE committers who do not have a github account', async () => {
        direct_call_data = testDataComments_withLICENSEComment
        assertionFunction = async (args) => {
            assert(args.arg.body.indexOf('**user1, user2** seem not to be a GitHub user. You need a GitHub account to be able to sign the LICENSE. ') >= 0)
            return 'githubRes'
        }
    ...
    it('should add a note to the commment for Corporate LICENSE and write a list of signed and not signed external committers on create', async () => {
        direct_call_data = []
        assertionFunction = async (args) => {
            assert.equal(args.fun, 'createComment')
            assert(args.arg.body.indexOf('sign our [Contributor License Agreement]') >= 0)
    ...
    it('should add a note for Corporate LICENSE and should NOT write a list of signed and not signed users on create if there is only one external committer ', async () => {
        direct_call_data = []
        assertionFunction = async (args) => {
            assert.equal(args.fun, 'createComment')
            assert(args.arg.body.indexOf('sign our [Contributor License Agreement]') >= 0)
...
describe('pullRequest:getComment', () => {
    beforeEach(() => {
        ...
        sinon.stub(github, 'call').callsFake(async (args) => {
            if (args.obj === 'issues' && args.fun === 'listComments') {
                assert.equal(args.token, 'xyz')
                return {
                    data: testDataComments_withLICENSEComment
                }
            }
        })
    })
    ...
    it('should get License Assistant_s comment', async () => {
        const args = {
            repo: 'myRepo',
            owner: 'owner',
            number: 1
        ...
        assert.deepEqual(comment, testDataComments_withLICENSEComment[1])
    })

    it('should not find the comment if it is not there', async () => {
        github.call.restore()
        sinon.stub(github, 'call').resolves({
            data: testDataComments_withoutLICENSE
        })
        const args = {
            repo: 'myRepo',
            owner: 'owner',
...
describe('pullRequest:deleteComment', () => {
    ...
    it('should delete comment', async () => {
        res.listComments = testDataComments_withLICENSEComment
        await pullRequest.deleteComment(args)
        assert(github.call.calledWithMatch({
            obj: 'issues',
            fun: 'deleteComment'
