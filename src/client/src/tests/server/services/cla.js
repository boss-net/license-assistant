...
//model
const LICENSE = require('../../../server/src/documents/cla').LICENSE
...
const stub = () => {
    ...
    sinon.stub(LICENSE, 'findOne').callsFake(async () => {
...
describe('cla:getLastSignature', () => {
    ...
    it('should get cla entry for equal repo, userId or user and gist url', async () => {
        ...
        assert.equal(LICENSE.findOne.calledWithMatch({
            $or: [{
                userId: 'userId',
                gist_url: 'url/gistId',
                gist_version: 'xyz',
    ...
    it('should update user name if github username is changed', async () => {
        ...
        LICENSE.findOne.restore()
        sinon.stub(LICENSE, 'findOne').callsFake(async (query) => {
    ...
    it('should positive check if a repo has a null LICENSE', async () => {
...
describe('cla:checkPullRequestSignatures', () => {
    ...
    it('should positive check if an repo has a null LICENSE', async () => {
        testRes.repoServiceGet.gist = undefined
        const args = {
            repo: 'myRepo',
            owner: 'owner',
    ...
    it('should return map of committers who has signed, who has not signed and who has no github account', async () => {
        ...
        LICENSE.findOne.restore()
        sinon.stub(LICENSE, 'findOne').callsFake(async (arg) => {
    ...
    it('should exclude committers on allowlist from the map', async () => {
        ...
        LICENSE.findOne.restore()
        sinon.stub(LICENSE, 'findOne').callsFake(async () => null)
    ...
    describe('organization signee', function () {
        ...
        it('should call callback function immediately if organization is on allowlist and there are no external committers ', async () => {
            ...
            try {
                ...
                assert(signed)
                sinon.assert.notCalled(LICENSE.findOne)
            } finally {
                config.server.feature_flag.required_signees = ''
            }
        })
        it('should check if the external committer has signed the LICENSE when the organization is on allowlist (external Organisation) ', async () => {
            config.server.feature_flag.required_signees = 'submitter committer'
            ...
            try {
                ...
                assert(signed)
                sinon.assert.called(LICENSE.findOne)
            } finally {
                config.server.feature_flag.required_signees = ''
            }
        })
...
describe('cla:sign', () => {
    ...
    beforeEach(() => {
        ...
        sinon.stub(LICENSE, 'create').callsFake((args) => {
            assert(args)
    ...
    it('should store signed cla data for org', async () => {
        ...
        assert(LICENSE.create.called)
        assert(LICENSE.create.calledWithMatch({
            gist_url: 'url/gistId'
        }))
        assert(org_service.get.called)
    })
    ...
    it('should do nothing if user has already signed', async () => {
        ...
        try {
            ...
            assert(error)
            assert.equal(LICENSE.create.called, false)
        }
    })
    ...
    it('should send error message when a repo linked with an Null LICENSE', async () => {
        testErr.claCreate = 'any DB error'
        testRes.claCreate = null
...
describe('cla:create', () => {
    afterEach(() => LICENSE.create.restore())

    it('should create cla entry for equal repo, user and gist url', async () => {
        sinon.stub(LICENSE, 'create').callsFake(async (arg) => {
            assert(arg)
            assert(arg.gist_url)
...
describe('cla:getSignedLICENSE', () => {
    it('should get all clas signed by the user but only one per repo (linked or not)', async () => {
        ...
        sinon.stub(LICENSE, 'find').callsFake(async () => {
            return [{
                repo: 'repo1',
                user: 'login',
                owner: 'owner1',
        ...
        const clas = await cla.getSignedLICENSE(args)
        assert.equal(clas.length, 3)
        assert.equal(clas[2].repo, 'repo3')
        LICENSE.find.restore()
    ...
    it('should select cla for the actual linked gist per repo even if it is signed earlier than others', async () => {
        ...
        assert.equal(LICENSE.find.callCount, 1)
        LICENSE.find.restore()
        Repository.find.restore()
        Org.find.restore()
    })
...
describe('cla:getAll', () => {
    beforeEach(() => {
        sinon.stub(LICENSE, 'find').callsFake(async (arg) => {
            assert(arg)
            let resp = [{
                id: 2,
                created_at: '2011-06-20T11:34:15Z',
    ...
    afterEach(() => LICENSE.find.restore())
    ...
    it('should get all signed cla with same orgId', async () => {
        ...
        assert.equal(LICENSE.find.calledWith({
            $or: [{
                ownerId: 1,
                gist_url: 'gistUrl'
            }]
    ...
    it('should get all signed cla with same repoId', async () => {
        ...
        const arr = await cla.getAll(args)
        assert.equal(LICENSE.find.calledWithMatch({
            $or: [{
                repoId: testData.repo.id,
                gist_url: 'gistUrl'
            }]
    ...
    it('should get all clas for shared gist repo/org', async () => {
        ...
        await cla.getAll(args)
        assert(LICENSE.find.calledWith({
            $or: [{
    ...
    it('should get all clas per user if gist_version provided', async () => {
        LICENSE.find.restore()
        sinon.stub(LICENSE, 'find').callsFake(async (arg, _prop, options) => {
            assert(arg)
            assert(options.sort)
            return [{
                id: 1,
...
describe('cla:isClaRequired', () => {
    ...
    it('should require a LICENSE when minimum changes don\'t set up', async () => {
        const claIsRequired = await cla.isClaRequired(args)

        assert(claIsRequired)
    })

    it('should require a LICENSE when pull request exceed minimum file changes', async () => {
    ...
    it('should require a LICENSE when pull request exceed minimum code changes', async () => {
    ...
    it('should NOT require a LICENSE when pull request NOT exceed minimum file and code changes', async () => {
        testRes.repoServiceGet.minFileChanges = 2
        testRes.repoServiceGet.minCodeChanges = 15
        testRes.getPR = {
            data: {
